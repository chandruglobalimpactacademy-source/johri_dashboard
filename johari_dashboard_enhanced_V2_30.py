...
# shift codes that cover a full day on their own — running one of these means
# nothing is "missed", same treatment as running AM + PM together.
FULL_DAY_SHIFTS = {"AS", "FULL SHIFT"}
...
def _find_header_row(raw, keyword="date", scan_rows=20):
    """Some exported sheets have title/route-info rows before the real header
    row (e.g. company name, 'Route No:', blank row, THEN the column headers).
    Scan the first `scan_rows` rows for one containing a cell that matches
    `keyword` and treat that as the header row. Falls back to row 0."""
    limit = min(scan_rows, len(raw))
    for i in range(limit):
        row_vals = raw.iloc[i].astype(str).str.strip().str.lower()
        if (row_vals == keyword).any():
            return i
    return 0


def _load_with_dynamic_header(raw):
    hi = _find_header_row(raw, keyword="date")
    out = raw.iloc[hi + 1:].copy()
    out.columns = raw.iloc[hi].tolist()
...
if uploaded_file.name.endswith("xlsx"):
    ...
    raw_main = xls.parse(xls.sheet_names[0], header=None)
    df = _load_with_dynamic_header(raw_main)
    ...
    raw_main = pd.read_csv(uploaded_file, header=None)
    df = _load_with_dynamic_header(raw_main)

# --- normalize incoming column headers: any case/spacing → canonical names app expects ---
COLUMN_ALIASES = {
    "date": "Date", "depot": "Depot", "route": "Route", "service": "Service",
    "type": "Type", "shift": "Shift",
    "collection": "Collection With Reimbursement",
    "collection with reimbursement": "Collection With Reimbursement",
    "epkm": "EPKM",
    "scheduled km": "Scheduled KM", "sch km": "Scheduled KM",
    "operated km": "Operated KM", "op km": "Operated KM",
    "km efficency": "KM Efficency", "km efficiency": "KM Efficency",
    "scheduled trip": "Scheduled Trip", "sch trip": "Scheduled Trip",
    "operated trip": "Operated Trip", "op trip": "Operated Trip",
...
if abstract_df is not None:
    abstract_df = _normalize_columns(abstract_df)

# optional columns: some source files only carry Total Passengers, not the
# Zero Value Ticket / Ticket Passenger / Senior Citizens breakdown — backfill w/ 0
# so downstream .agg() calls (which reference them by name) don't KeyError.
OPTIONAL_COLS_DEFAULT = {"Zero Value Ticket": 0, "Ticket Passenger": 0, "Senior Citizens": 0}
missing_optional = [c for c in OPTIONAL_COLS_DEFAULT if c not in df.columns]
for col, default_val in OPTIONAL_COLS_DEFAULT.items():
    if col not in df.columns:
        df[col] = default_val
if missing_optional:
    st.info(
        f"Uploaded file has no {', '.join(missing_optional)} column(s) — "
        f"showing as 0 in this dashboard."
    )

REQUIRED_COLS = ["Date", "Depot", "Route", "Service", "Type", "Shift",
                  "Collection With Reimbursement", "EPKM", "Scheduled KM",
                  "Operated KM", "KM Efficency", "Scheduled Trip", "Operated Trip",
                  "Total Passengers"]
missing = [c for c in REQUIRED_COLS if c not in df.columns]
if missing:
    st.error(
        f"Uploaded file missing required column(s): {', '.join(missing)}. "
...
if active_tab == TAB_LABELS[0]:
    ...
    def not_operated_for(g):
        # if this service only ever uses a full-day shift code (never AM/PM),
        # a missed day means the bus simply didn't run — no sub-shift to name.
        all_shifts_seen = set(g["Shift"].unique())
        full_day_only = bool(all_shifts_seen) and all_shifts_seen.issubset(FULL_DAY_SHIFTS)

        by_date = g.groupby("Date")["Shift"].apply(set)
        entries = []
        for d in full_range:
            present = by_date.get(d, set())
            if present & FULL_DAY_SHIFTS:
                continue  # full-day shift ran — nothing is missed
            if full_day_only:
                if not present:
                    entries.append(d.strftime("%d-%b"))
            else:
