...
# --- best-effort holiday list — EDIT HERE to add/correct dates for your state/year.
# This is a small hardcoded list, not a live calendar, so treat it as a starting point. ---
HOLIDAYS = {
    "2025-04-06": "Ram Navami",
    "2025-04-10": "Mahavir Jayanti",
    "2025-04-14": "Tamil New Year / Ambedkar Jayanti",
...
# ------------------------------------------------------------------
# TAB 1 — JOHARI WINDOW (rich hover card)
# ------------------------------------------------------------------
with tab1:
    st.subheader("Johari Window of Services – Avg EPKM vs EPBPD")

    # --- best-effort holiday list — see HOLIDAYS near the top of the file to edit ---
    st.caption("Hover a point for the full breakdown. Holiday flags use a small hardcoded list "
               "(edit `HOLIDAYS` near the top of the code for your state/year) — verify before relying on it.")
...
# ------------------------------------------------------------------
# TAB 2 — TRENDS OVER TIME (rich hover when broken down by Service/Depot)
# ------------------------------------------------------------------
with tab2:
    ...
    if group_by in ("Service", "Depot"):
        ...
        fig2 = px.line(base, x="Date", y="value", color=group_by, markers=True, custom_data=custom_cols)

        fig2.update_traces(hovertemplate=(
            f"<b>{group_by}:</b> %{{fullData.name}}  ({other_label}: %{{customdata[0]}})<br>"
