...
# ------------------------------------------------------------------
    ...
    if focus_service:
        ...
        bc1.info(f"Showing only Service **'{focus_service}'** (selected from the Johari Window). "
                 "Use the box below to add other services back.")
    ...
    if group_by == "Service":
        service_options_here = sorted(base["Service"].unique())
        ...
        base = base[base["Service"].isin(chosen_services)]
        if base.empty:
    ...
    # --- color individual markers: red = govt holiday, orange = Sunday, else the trace's own color ---
    holiday_idx = 4 if group_by in ("Service", "Depot") else 1
    weekday_idx = 3 if group_by in ("Service", "Depot") else 0
    for trace in fig2.data:
        ...
        for cd in trace.customdata:
            if cd[holiday_idx] != "No":
                ...
                colors.append("orange"); sizes.append(9)
            else:
    ...
    st.caption("🔴 marker = Tamil Nadu Govt holiday · 🟠 marker = Sunday · normal color = regular weekday.")
