...
# ------------------------------------------------------------------
    ...
    _obs_metrics = {
        ...
        "Avg EPBPD (₹/bus/day)": ("EPBPD", True, "₹{:,.0f}"),
        "KM Efficiency (%)": ("Efficiency", True, "{:.1f}%"),
        "Avg Passengers/bus/day": ("PaxPerBus", True, "{:,.1f}"),
        "Total Collection (₹)": ("Collection", True, "₹{:,.0f}"),
        "Total Passengers": ("Pax", True, "{:,.0f}"),
        "Avg services operated/day": ("Services", True, "{:.1f}"),
        "Avg trips operated/day": ("TripsPerDay", True, "{:.1f}"),
        "Zero Value Tickets (total)": ("ZeroTicket", False, "{:,.0f}"),   # lower is better
    ...
    for i, (label, (col, higher_better, fmt)) in enumerate(_obs_metrics.items()):
        ...
        with _cards[i % 3]:
            st.markdown(
                ...
                f"<div style='font-size:1rem;color:#222;'>{fmt.format(row[col])}"
                f"{' (lowest)' if not higher_better else ''}</div>"
    ...
    st.caption("EPKM, EPBPD and KM Efficiency use totals-based math. 'Best' picks the highest value per category, "
               "except Zero Value Tickets where lowest is best.")
