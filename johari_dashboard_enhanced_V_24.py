...
TAB_LABELS = ["🎯 Johari Window", "📈 Trends", "🏢 Depot Comparison", "📋 Data", "📆 Daily Abstract"]
...
if active_tab == TAB_LABELS[0]:
    ...
    st.markdown(f"**Total No. of Services:** {_total_services} &nbsp;|&nbsp; **Total No. of Depots:** {_total_depots}")
    st.markdown(" &nbsp;|&nbsp; ".join(
        f"**{depot_emoji.get(d,'')} {d} – No. of Services:** {c}" for d, c in _dep_svc_counts.items()
    ))
    st.markdown(
        f"**Avg. EPBPD:** ₹{_avg_epbpd:,.0f} &nbsp;|&nbsp; **Avg. EPKM:** ₹{weighted_epkm:.2f} &nbsp;|&nbsp; "
        f"**Avg. No. of buses/day:** {_avg_buses_per_day:.1f} &nbsp;|&nbsp; "
        f"**Scheduled kms per bus/day:** {_sched_km_per_bus_day:,.1f} &nbsp;|&nbsp; "
        f"**Avg. KM efficiency:** {weighted_eff:.1f}%"
    )
    ...
    def not_operated_for(g):
        all_shifts_here = set(g["Shift"].unique())
        by_date = g.groupby("Date")["Shift"].apply(set)
        entries = []
        for d in full_range:
            present = by_date.get(d, set())
            missing = all_shifts_here - present
            if missing:
    ...
    if st.session_state.get("trend_focus_service"):
        fc1, fc2 = st.columns([4, 1])
        fc1.info(f"Currently focused on '{st.session_state['trend_focus_service']}' in the Trends section.")
        if fc2.button("Clear selection"):
            st.session_state.pop("trend_focus_service", None)
            st.session_state.pop("trend_services_multiselect", None)
            st.rerun()
...
# ------------------------------------------------------------------
    ...
    if metric == "Combined EPKM + EPBPD":
        ...
        def _shift_block(g):
            ...
            for r in g.sort_values("Shift").itertuples():
                blocks.append(
                    ...
                    f"Scheduled KM: {r.SchKM:,.1f} &nbsp; Operated KM: {r.OpKM:,.1f}<br>"
                    f"Scheduled Trips: {r.SchTrip:,.0f} &nbsp; Operated Trips: {r.Trips:,.0f}<br>"
        ...
        # --- per-date shift status (dynamic): same rule as Johari Window's NotOperated —
        # "expected" shifts = whatever shifts this scoped selection actually runs, not a fixed list ---
        scope_shifts = set(scope_df["Shift"].unique())
        def _shift_status(g):
            present = sorted(g["Shift"].unique())
            if not present:
                return "Not run (no shifts operated)"
            missing = sorted(scope_shifts - set(present))
            txt = "Running: " + ", ".join(present)
            txt += (" — Not running: " + ", ".join(missing)) if missing else " — (all shifts running)"
            return txt
        ...
        combined_block = (
            ...
            "Scheduled KM: %{customdata[5]:,.1f} &nbsp; Operated KM: %{customdata[6]:,.1f}<br>"
            "Scheduled Trips: %{customdata[7]:,.0f} &nbsp; Operated Trips: %{customdata[8]:,.0f}<br>"
        ...
        epbpd_colors = ["gold" if s else "#1E90FF" for s in sunday_mask]   # bright dodger blue
        epkm_colors = ["gold" if s else "#FF3131" for s in sunday_mask]    # bright red
        marker_sizes = [11 if s else 8 for s in sunday_mask]
        ...
        figc.update_layout(
            ...
            yaxis2=dict(title=dict(text="EPKM (₹/km)", font=dict(color="red")), tickfont=dict(color="red"),
        ...
        st.caption("Combined view aggregates the currently scoped selection by day "
                   ...
                   "🟡 gold marker = Sunday. Green dashed = this selection's avg EPBPD, "
        ...
        if group_by in ("Service", "Depot"):
            ...
            fig2.update_traces(hovertemplate=(
                ...
                "Scheduled KM: %{customdata[8]:,.1f} &nbsp; Operated KM: %{customdata[9]:,.1f}<br>"
                "Scheduled Trips: %{customdata[10]:,.0f} &nbsp; Operated Trips: %{customdata[11]:,.0f}<br>"
