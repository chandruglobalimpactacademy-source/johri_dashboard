...
with tab3:
    ...
    dep_stats = df.groupby("Depot")["KM Efficency"].agg(mean="mean", min="min", max="max").reset_index()
    fig4 = go.Figure()
    fig4.add_trace(go.Bar(
        x=dep_stats["Depot"], y=dep_stats["mean"],
        error_y=dict(type="data", symmetric=False,
                     array=dep_stats["max"] - dep_stats["mean"],
                     arrayminus=dep_stats["mean"] - dep_stats["min"],
                     color="gray", thickness=1.5, width=5),
        marker_color="#1f77b4",
        text=dep_stats["mean"].round(1).astype(str) + "%",
        textposition="outside",
    ))
    fig4.add_hline(y=85, line_dash="dot", line_color="red", annotation_text="85% min target")
    fig4.update_layout(
        title="KM Efficiency by Depot — average with day-to-day range",
        yaxis_title="KM Efficiency (%)", xaxis_title="Depot",
        plot_bgcolor="white", paper_bgcolor="white", showlegend=False,
    )
    c2.plotly_chart(fig4, use_container_width=True)
    c2.caption("Bar = average. Whiskers = lowest and highest day observed in the filtered range.")
